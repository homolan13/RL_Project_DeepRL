...
class Actor(th.nn.Module):
    ...
    def __init__(self, state_dimension, action_dimension, max_action):
        ...
        self.drnn = drnn.DRNN(state_dimension, 32, 3, dropout=0, cell_type="GRU", batch_first=True)
        self.linear = th.nn.Linear(32, 1)
...
class Critic(th.nn.Module):
    ...
    def __init__(self, state_dimension, action_dimension):
        ...
        self.drnn = drnn.DRNN(state_dimension+1, 32, 3, dropout=0, cell_type="GRU", batch_first=True)
        self.linear = th.nn.Linear(32, action_dimension)

    def forward(self, state, action):
        x  = th.cat([state, action], 2)
        x, _ = self.drnn(x)
        x = self.linear(x)
        return x
...
class Trainer:
    ...
    def __init__(self, config_file, enable_logging):
        ...
        # Load weights
        #self.agent.load_checkpoint("models/DDPG_simglucose-basal_0")
        ...
        for ts in tqdm(range(1, int(self.config['num_episodes']) + 1)):
            ...
            for episode_duration in range(max_episode_timesteps):
                ...
                if ts < self.config['start_time_step']:
                    ...
                    action_ = self.agent.select_action(state_)
                    action = round(action_[0])
                
                next_state, reward, done, _, info = self.env.step(action)
                self.memory.add(state, action_, next_state, reward, float(done), episode_duration)
                state = next_state
...
def register_gym():
    ...
    for i in range(100):
        ...
        start_time = dt.datetime(2018, 1, 1, 6, minute, 0) 
        scenarios.append(CustomScenario(start_time=start_time, scenario=[(0.5, 100)]))
